...
if uploaded_files and st.sidebar.button("Process Documents"):
    with st.spinner(f"Processing {len(uploaded_files)} manual(s)... Extracting logical page labels."):
        ...
        for uploaded_file in uploaded_files:
            ...
            # Attempt to grab the publisher's embedded page labels (e.g., "i", "ii", "1", "52")
            ...
            with st.spinner("Searching technical manuals..."):
                ...
                def format_docs(docs):
                    ...
                    for doc in docs:
                        ...
                        # We removed the +1 here because our custom extraction handles the exact string
                        page = doc.metadata.get("page", "Unknown") 
                        formatted.append(f"--- SOURCE: {source} (Page {page}) ---\n{doc.page_content}")
                    return "\n\n".join(formatted)

                template = """You are an expert Biomedical Equipment Assistant. Use the retrieved context to answer the technician's question.
